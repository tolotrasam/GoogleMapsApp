...
import ca.joel.mapapp.model.Bus;
...
public class MainActivity extends FragmentActivity implements LocationListener,
    ...
    private DatabaseReference firebaseDB_User;
    private DatabaseReference firebaseDB_Bus_Stops;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        firebaseDB_User.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                ...
                for (DataSnapshot bus : dataSnapshot.getChildren()) {
                    ...
                    Bus myBus = null;
                    myBus = bus.getValue(Bus.class);
                    LatLng coord = new LatLng(myBus.getLat(), myBus.getLon());
                    googleMap.addMarker(new MarkerOptions().position(coord).title("Bus Location"));
//
//                    if (bus.child("Lon").exists() && bus.child("Lat").exists()) {
//                        double lat = bus.child("Lat").getValue(double.class);
//                        double lon = bus.child("Lon").getValue(double.class);
//                        LatLng coord = new LatLng(lat, lon);
//                        googleMap.addMarker(new MarkerOptions().position(coord).title("Bus Location"));
//                        Log.d("hello", "updated coord");
//                    } else {
//                        Log.d("hello", "something wrong");
//                    }
                }
                for(Bus bus : Buses) {
                    if(bus.getLastBusStop().coord
...
//                    }
        ...
        firebaseDB_Bus_Stops = FirebaseDatabase.getInstance().getReference("bus_stops");
        //Get loop from Kevin
        markerOptions.addEventListener(new EventListener() {
            @Override
            public void onMarkerClick(final Marker marker) {
                double fastestBus;

                for (DatabaseReference bus:Buses) {
                    if (getDistanceBetweenTwoPoints(bus.coord, busStop.coord, bus.waypointsUPTOdestination) < fastestBus) {
                        fastestBus = getDistanceBetweenTwoPoints()
                    }
                }
            }
        }
